...
class ImportCommand extends Command
{
    ...
    private function renderImportResults(ProductImport $import): void
    {
        $options = [
            ...
            'errors' => count($import->getFailedRows()),
        ];

        $failedRowsHtml = '';
        foreach ($import->getFailedRows() as $failedRow) {
            $rowNumber = $failedRow['Row'] ?? 'N/A';
            $productCode = $failedRow['Product Code'] ?? 'N/A';
            $messages = $failedRow['Messages'] ?? [];
            $messagesHtml = implode('<br>', array_map('htmlspecialchars', $messages));

            $failedRowsHtml .= <<<HTML
                <div class="mt-1">
                    <strong>Row:</strong> $rowNumber <br>
                    <strong>Product Code:</strong> $productCode <br>
                    <strong>Erros:</strong> <br>
                    <span class="text-red-500">$messagesHtml</span>
                </div>
            HTML;
        }
        ...
        render(<<<HTML
            <div class="mb-1">
                ...
                <div class="px-1">
                    ...
                    <div class="mt-1">
                        <div class="px-2 bg-red-500 font-bold text-black">FAILED ROWS</div>
                        <div class="px-1">
                            $failedRowsHtml
                        </div>
                    </div>
